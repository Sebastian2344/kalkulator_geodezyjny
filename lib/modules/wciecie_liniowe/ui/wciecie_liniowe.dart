...
import 'save_alert_dialog_wl.dart';

class WciecieLiniowe extends StatefulWidget {
  const WciecieLiniowe({super.key});

  @override
  State<WciecieLiniowe> createState() => _WciecieLinioweState();
}

class _WciecieLinioweState extends State<WciecieLiniowe> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Wcięcie Liniowe'),
        centerTitle: true,
        automaticallyImplyLeading: false,
        actions: [
          SaveAlertDialogWL(controller7: controller8)
        ],
      ),
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                children: [
                  _buildColumnForData(controller5, "Odległość AP"),
                  _buildColumnForData(controller6, "Odległość BP")
                ],    
              ),_buildColumnForData(controller7, "Odległość AB"),
              FilledButton(
                  ...
                  onPressed: () {
                    context.read<WciecieLinioweCubit>().oblicz(
                        controller.text,
                        controller2.text,
                        controller3.text,
                        controller4.text,
              ...
              BlocBuilder<WciecieLinioweCubit, WciecieLinioweState>(
                builder: (context, state) {
                  if (state is WciecieLinioweError) {
                  ...
                  } else if (state is WciecieLinioweSuccessful) {
                    return Table(
                      columnWidths: const {
                        0: FlexColumnWidth(4),
                        1: FlexColumnWidth(5),
                      ...
                      children: [
                        _buildTableRow([
                          "Współżędne:",
                          state.wciecieLinioweModel.xP,
                          state.wciecieLinioweModel.yP
                        ]),
                        _buildTableRow([
                          "Odległości Ap i Bp:",
                          state.wciecieLinioweModel.ap,
                          state.wciecieLinioweModel.bp
                        ])
                      ],
                    );
                  } else {
