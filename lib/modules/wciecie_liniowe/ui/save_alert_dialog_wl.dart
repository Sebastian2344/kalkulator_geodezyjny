...
class SaveAlertDialogWL extends StatelessWidget {
  const SaveAlertDialogWL({super.key, required this.controller7});
  final TextEditingController controller7;
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<WciecieLinioweCubit, WciecieLinioweState>(
      builder: (context, state) => IconButton(
        icon: const Icon(Icons.save),
        onPressed: () async {
          final save = context.read<CrudWciecieLinioweCubit>();
          if (state is WciecieLinioweSuccessful) {
            showDialog(
              context: context,
              builder: (context) {
                return AlertDialog(
                      ...
                      child: ElevatedButton(
                          onPressed: () async {
                            if (controller7.text != '') {
                              await save.openDB();
                              await save.saveData(
                                  state.wciecieLinioweModel, controller7.text);
                              await save.getData();
                            }
                          },
